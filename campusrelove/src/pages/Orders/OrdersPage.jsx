...
function OrderCard({ order, role, onAction }) {
  ...
  const st = STATUS_LABEL[order.status] || STATUS_LABEL.pending_payment
    ...
    <div className={styles.orderCard}>
      <div className={styles.orderHeader}>
        <div className={styles.orderMeta}>
          ...
          <span className={styles.orderDate}>{new Date(order.createdAt).toLocaleDateString('id-ID', { day:'numeric', month:'short', year:'numeric' })}</span>
        </div>
      ...
      {/* Escrow info — tampil untuk buyer saat status paid/processing/shipped */}
      ...
      {/* Dana breakdown — tampil untuk seller saat completed */}
      {role === 'seller' && order.status === 'completed' && (
        <div className={styles.fundBreakdown}>
          ...
          <div className={styles.fundRow + ' ' + styles.fundTotal}>
            ...
            <strong>Rp {sellerAmt.toLocaleString('id-ID')}</strong>
          </div>
        </div>
      )}
      ...
      <div className={styles.orderActions}>
        {/* BUYER: konfirmasi terima → langsung selesai + dana cair otomatis */}
            ...
            {!showConfirm ? (
              ...
              <div className={styles.confirmBox}>
                ...
                <p className={styles.confirmNote}>
                  Dana <strong>Rp {sellerAmt.toLocaleString('id-ID')}</strong> akan langsung cair ke penjual.
                ...
                <div className={styles.confirmActions}>
                  <button className={styles.btnGreen} onClick={() => {
                    onAction('confirmDelivery', order.orderId)
        ...
        {role === 'seller' && order.status === 'paid' && (
          <>
            ...
            {!showReject ? (
              <button className={styles.btnRed} onClick={() => setShowReject(true)}>
                ❌ Tolak
              </button>
            ) : (
        ...
        {role === 'seller' && order.status === 'processing' && (
          <>
            {!showShip ? (
              <button className={styles.btnPrimary} onClick={() => setShowShip(true)}>
                🚚 Input Pengiriman
              </button>
            ) : (
              <div className={styles.shipForm}>
                <div className={styles.shipMethodRow}>
                  <button
                    className={`${styles.shipMethodBtn} ${shipForm.method === 'cod' ? styles.shipMethodActive : ''}`}
                    onClick={() => setShipForm({...shipForm, method: 'cod'})}
                  >📍 COD</button>
                  <button
                    className={`${styles.shipMethodBtn} ${shipForm.method === 'ekspedisi' ? styles.shipMethodActive : ''}`}
                    onClick={() => setShipForm({...shipForm, method: 'ekspedisi'})}
                  >📦 Ekspedisi</button>
                </div>
                {shipForm.method === 'ekspedisi' ? (
                  ...
                  <input
                    placeholder="Jadwal COD (misal: Sabtu 10 Jan, 14:00 di Perpus)"
        ...
        <Link to={`/product/${order.productId}`} className={styles.btnOutline}>
          👁 Lihat Produk
        </Link>
      </div>
    </div>
      ...
      <div className="container">
        ...
        {filtered.length === 0 ? (
          <div className={styles.empty}>
            ...
            {user.role === 'buyer' && (
              <Link to="/browse" className={styles.btnPrimary}>🛍️ Browse Barang</Link>
            )}
          </div>
        ) : (
